/*
 * ! jQuery UI - v1.10.3 - 2013-05-03 http://jqueryui.com Includes:
 ...
 * jquery.ui.effect-slide.js, jquery.ui.effect-transfer.js, jquery.ui.menu.js,
 * jquery.ui.position.js, jquery.ui.progressbar.js, jquery.ui.slider.js,
 * jquery.ui.spinner.js, jquery.ui.tabs.js, jquery.ui.tooltip.js Copyright 2013
...
(function($, undefined) {
	...
	$.fn.extend({
				focus : (function(orig) {
					return function(delay, fn) {
						return typeof delay === "number" ? this.each(
								function() {
									var elem = this;
									setTimeout(function() {
												$(elem).focus();
	...
	$.fn.extend({
				disableSelection : function() {
					return this.bind(($.support.selectstart
									? "selectstart"
									: "mousedown")
		...
		_mouseUp : function(event) {
			...
			if (this._mouseStarted) {
				...
				if (event.target === this._mouseDownEvent.target) {
					$.data(event.target,
							this.widgetName + ".preventClickEvent", true);
				}
...
(function($, undefined) {
	...
	$.widget("ui.draggable", $.ui.mouse, {
		...
		_mouseStop : function(event) {
			...
			// continue (see #8269)
			if (this.options.helper === "original"
					&& !$.contains(this.element[0].ownerDocument,
							this.element[0])) {
				return false;
			}
		...
		_getHandle : function(event) {
			return this.options.handle ? !!$(event.target).closest(this.element
					.find(this.options.handle)).length : true;
		},

		_createHelper : function(event) {

			var o = this.options, helper = $.isFunction(o.helper) ? $(o.helper
					.apply(this.element[0], [event])) : (o.helper === "clone"
		...
		_convertPositionTo : function(d, pos) {
			...
			var mod = d === "absolute" ? 1 : -1, scroll = this.cssPosition === "absolute"
					&& !(this.scrollParent[0] !== document && $.contains(
			...
			return {
				...
				((this.cssPosition === "fixed" ? -this.scrollParent
						.scrollLeft() : this.offset.scroll.left) * mod))
			};
	...
	$.ui.plugin.add("draggable", "connectToSortable", {
		...
		stop : function(event, ui) {
			...
			var inst = $(this).data("ui-draggable"), uiSortable = $.extend({},
					ui, {
						item : inst.element
					});
			...
			$.each(inst.sortables, function() {
				...
				if (innermostIntersecting) {
					...
					if (!this.instance.isOver) {
						...
						this.instance.currentItem = $(that).clone()
								.removeAttr("id")
	...
	$.ui.plugin.add("draggable", "snap", {
		...
		drag : function(event, ui) {
			...
			var ts, bs, ls, rs, l, r, t, b, i, first, inst = $(this)
					.data("ui-draggable"), o = inst.options, d = o.snapTolerance, x1 = ui.offset.left, x2 = x1
	...
	$.ui.plugin.add("draggable", "stack", {
				start : function() {
					var min, o = this.data("ui-draggable").options, group = $
							.makeArray($(o.stack)).sort(function(a, b) {
...
(function($, undefined) {
	...
	$.ui.ddmanager = {
		...
		prepareOffsets : function(t, event) {
			...
			var i, j, m = $.ui.ddmanager.droppables[t.options.scope] || [], type = event
					? event.type
...
(function($, undefined) {
	...
	$.widget("ui.resizable", $.ui.mouse, {
		...
		_create : function() {
			...
			if (this.element[0].nodeName
				...
				// Create a wrapper element and set the wrapper to the new
				// current internal element
				this.element
						.wrap(	$("<div class='ui-wrapper' style='overflow: hidden;'></div>")
										.css({
				...
				// Push the actual element to our proportionallyResize internal
				// array
				...
				for (i in this.handles) {
					...
					if (this.elementIsWrapper
						...
						padWrapper = /sw|ne|nw|se|n|s/.test(i) ? axis
								.outerHeight() : axis.outerWidth();

						// The padding type i have to apply...
						padPos = [
								"padding",
								/ne|nw|n/.test(i) ? "Top" : /se|sw|s/.test(i)
										? "Bottom"
										: /^e$/.test(i) ? "Right" : "Left"]
								.join("");
					...
					// TODO: What's that good for? There's not anything to be
					// executed left
			...
			if (o.autoHide) {
				this._handles.hide();
				$(this.element).addClass("ui-resizable-autohide").mouseenter(
						function() {
							if (o.disabled) {
								return;
							}
		...
		_mouseDrag : function(event) {
			...
			var data, el = this.helper, props = {}, smp = this.originalMousePosition, a = this.axis, prevTop = this.position.top, prevLeft = this.position.left, prevWidth = this.size.width, prevHeight = this.size.height, dx = (event.pageX - smp.left)
					|| 0, dy = (event.pageY - smp.top) || 0, trigger = this._change[a];
		...
		_respectSize : function(data) {
			...
			var o = this._vBoundaries, a = this.axis, ismaxw = isNumber(data.width)
					...
					&& o.maxHeight && (o.maxHeight < data.height), isminw = isNumber(data.width)
					&& o.minWidth && (o.minWidth > data.width), isminh = isNumber(data.height)
					&& o.minHeight && (o.minHeight > data.height), dw = this.originalPosition.left
			...
			for (i = 0; i < this._proportionallyResizeElements.length; i++) {
				...
				prel.css({
					height : (element.height() - this.borderDif[0] - this.borderDif[2])
							|| 0,
					width : (element.width() - this.borderDif[1] - this.borderDif[3])
							|| 0
				});
	...
	$.ui.plugin.add("resizable", "animate", {
		...
		stop : function(event) {
			var that = $(this).data("ui-resizable"), o = that.options, pr = that._proportionallyResizeElements, ista = pr.length
					...
					&& $.ui.hasScroll(pr[0], "left") /* TODO - jump height */
					? 0
					: that.sizeDiff.height, soffsetw = ista
					? 0
	...
	$.ui.plugin.add("resizable", "containment", {
		...
		start : function() {
			var element, p, co, ch, cw, width, height, that = $(this)
					.data("ui-resizable"), o = that.options, el = that.element, oc = o.containment, ce = (oc instanceof $)
					? oc.get(0)
		...
		resize : function(event) {
			var woset, hoset, isParent, isOffsetRelative, that = $(this)
					.data("ui-resizable"), o = that.options, co = that.containerOffset, cp = that.position, pRatio = that._aspectRatio
					|| event.shiftKey, cop = {
				top : 0,
				left : 0
			...
			isParent = that.containerElement.get(0) === that.element.parent()
					.get(0);
			isOffsetRelative = /relative|absolute/.test(that.containerElement
					.css("position"));
		...
		stop : function() {
			var that = $(this).data("ui-resizable"), o = that.options, co = that.containerOffset, cop = that.containerPosition, ce = that.containerElement, helper = $(that.helper), ho = helper
					.offset(), w = helper.outerWidth() - that.sizeDiff.width, h = helper
					.outerHeight()
					- that.sizeDiff.height;
	...
	$.ui.plugin.add("resizable", "alsoResize", {
		...
		resize : function(event, ui) {
			...
			_alsoResize = function(exp, c) {
				$(exp).each(function() {
					var el = $(this), start = $(this)
							.data("ui-resizable-alsoresize"), style = {}, css = c
							&& c.length
							? c
	...
	$.ui.plugin.add("resizable", "grid", {
		...
		resize : function() {
			var that = $(this).data("ui-resizable"), o = that.options, cs = that.size, os = that.originalSize, op = that.originalPosition, a = that.axis, grid = typeof o.grid === "number"
					...
					* gridX, oy = Math.round((cs.height - os.height) / gridY)
					* gridY, newWidth = os.width + ox, newHeight = os.height
					+ oy, isMaxWidth = o.maxWidth && (o.maxWidth < newWidth), isMaxHeight = o.maxHeight
		...
		_mouseStart : function(event) {
			...
			$(event.target).parents().addBack().each(function() {
				...
				if (selectee) {
					...
					selectee.$element.removeClass(doSelect
							? "ui-unselecting"
							: "ui-selected").addClass(doSelect
							? "ui-selecting"
							: "ui-unselecting");
...
(function($, undefined) {
	...
	$.widget("ui.sortable", $.ui.mouse, {
		...
		_mouseStart : function(event, overrideHandle, noActivation) {
			...
			// We only need to call refreshPositions, because the refreshItems
			// call has been moved to mouseCapture
			...
			// playing any role during the drag, won't cause anything bad this
			// way
			...
			this._mouseDrag(event); // Execute the drag once - this causes the
									// helper not to be visible before getting
		...
		_mouseDrag : function(event) {
			...
			for (i = this.items.length - 1; i >= 0; i--) {
				...
				// Only put the placeholder inside the current Container, skip
				// all
		...
		cancel : function() {
			...
			if (this.placeholder) {
				...
				// way - unfortunately, it unbinds ALL events from the original
				// node!
		...
		_intersectsWith : function(item) {
			...
			var x1 = this.positionAbs.left, x2 = x1
					...
					+ item.width, t = item.top, b = t + item.height, dyClick = this.offset.click.top, dxClick = this.offset.click.left, isOverElementHeight = (this.options.axis === "x")
		...
		_intersectsWithPointer : function(item) {
			...
			var isOverElementHeight = (this.options.axis === "x")
					|| isOverAxis(this.positionAbs.top + this.offset.click.top,
							item.top, item.height), isOverElementWidth = (this.options.axis === "y")
		...
		_intersectsWithSides : function(item) {
			...
			var isOverBottomHalf = isOverAxis(this.positionAbs.top
					...
					this.positionAbs.left + this.offset.click.left, item.left
		...
		_contactContainers : function(event) {
			...
			for (i = this.containers.length - 1; i >= 0; i--) {
				...
				// never consider a container that's located within the item
				// itself
				...
				if (this._intersectsWith(this.containers[i].containerCache)) {
					...
					// if we've already found a container and it's more "inner"
			...
			if (this.containers.length === 1) {
				...
				for (j = this.items.length - 1; j >= 0; j--) {
					...
					if (Math.abs(cur - base) > Math.abs(cur
							+ this.items[j][sizeProperty] - base)) {
						nearBottom = true;
				...
				itemWithLeastDistance ? this._rearrange(event,
						...
						true);
				this._trigger("change", event, this._uiHash());
				this.containers[innermostIndex]._trigger("change", event, this
								._uiHash(this));
				...
				this.containers[innermostIndex]._trigger("over", event, this
								._uiHash(this));
				this.containers[innermostIndex].containerCache.over = 1;
			}
		...
		_createHelper : function(event) {
			...
			var o = this.options, helper = $.isFunction(o.helper)
		...
		_rearrange : function(event, i, a, hardRefresh) {
			...
			// the timeout, if it's still the same
	...
	(function(jQuery, undefined) {
		...
		color.fn = jQuery.extend(color.prototype, {
				...
				if (type === "object") {
					if (red instanceof color) {
						...
						each(spaces, function(spaceName, space) {
									...
									if (inst[cache]
											&& jQuery.inArray(null, inst[cache]
															.slice(0, 3)) < 0) {
			...
			transition : function(other, distance) {
				...
				each(space.props, function(key, prop) {
					...
					if (startValue === null) {
						...
						result[index] = clamp((endValue - startValue)
			...
			toRgbaString : function() {
				var prefix = "rgba(", rgba = jQuery.map(this._rgba, function(v,
								i) {
			...
			toHslaString : function() {
				var prefix = "hsla(", hsla = jQuery.map(this.hsla(), function(
								v, i) {
							if (v == null) {
								v = i > 2 ? 1 : 0;
							}
		...
		spaces.hsla.from = function(hsla) {
			...
			var h = hsla[0] / 360, s = hsla[1], l = hsla[2], a = hsla[3], q = l <= 0.5
					? l * (1 + s)
					: l + s - l * s, p = 2 * l - q;
		...
		each(spaces, function(spaceName, space) {
			...
			color.fn[spaceName] = function(value) {
				...
				var ret, type = jQuery.type(value), arr = (type === "array" || type === "object")
						? value
			...
			each(props, function(key, prop) {
				...
				color.fn[key] = function(value) {
					var vtype = jQuery.type(value), fn = (key === "alpha"
					...
					set : function(elem, value) {
						...
						if (value !== "transparent"
							...
							if (!support.rgba && value._rgba[3] !== 1) {
								...
								value = value.blend(backgroundColor
	...
	(function() {
		...
		$.effects.animateClass = function(value, duration, easing, callback) {
			...
			return this.queue(function() {
						var animated = $(this), baseClass = animated
								.attr("class")
								|| "", applyClassChange, allAnimations = o.children
						...
						$.when.apply($, allAnimations.get()).done(function() {
									...
									// jQuery.speed()
									// it also handles dequeuing the next
									// anim...
		...
		$.fn.extend({
					addClass : (function(orig) {
						return function(classNames, speed, easing, callback) {
							return speed ? $.effects.animateClass.call(this, {
										add : classNames
									}, speed, easing, callback) : orig.apply(
									this, arguments);
						};
					})($.fn.addClass),
					...
					toggleClass : (function(orig) {
						return function(classNames, force, speed, easing,
								callback) {
	...
	(function() {
		...
		$.extend($.effects, {
			...
			createWrapper : function(element) {
				...
				}, wrapper = $("<div></div>").addClass("ui-effects-wrapper")
						.css({
									fontSize : "100%",
	...
	$.widget("ui.accordion", {
		...
		_createIcons : function() {
			...
			if (icons) {
				$("<span>").addClass("ui-accordion-header-icon ui-icon "
						+ icons.header).prependTo(this.headers);
		...
		_destroy : function() {
			...
			this.headers
					...
					.removeAttr("aria-controls").removeAttr("tabIndex").each(
							function() {
		...
		_setOption : function(key, value) {
			...
			if (key === "disabled") {
				this.headers.add(this.headers.next()).toggleClass(
		...
		_eventHandler : function(event) {
			var options = this.options, active = this.active, clicked = $(event.currentTarget), clickedIsActive = clicked[0] === active[0], collapsing = clickedIsActive
					&& options.collapsible, toShow = collapsing ? $() : clicked
					.next(), toHide = active.next(), eventData = {
				oldHeader : active,
				oldPanel : toHide,
		...
		_toggle : function(data) {
			var toShow = data.newPanel, toHide = this.prevShow.length
					? this.prevShow
					: data.oldPanel;
		...
		_toggleComplete : function(data) {
			...
			toHide.removeClass("ui-accordion-content-active").prev()
...
(function($, undefined) {
	...
	$.widget("ui.autocomplete", {
		...
		_create : function() {
			...
			this.valueMethod = this.element[isTextarea || isInput
					? "val"
					: "text"];
			this.isNewMenu = true;
			...
			this._on(this.element, {
						keydown : function(event) {
							...
							switch (event.keyCode) {
								...
								default :
									suppressKeyPressRepeat = true;
									// search timeout should be triggered before
			...
			this._initSource();
			this.menu = $("<ul>").addClass("ui-autocomplete ui-front")
	...
	$.widget("ui.button", {
		...
		_create : function() {
			...
			var that = this, options = this.options, toggleButton = this.type === "checkbox"
					|| this.type === "radio", activeClass = !toggleButton
					? "ui-state-active"
					: "", focusClass = "ui-state-focus";

			if (options.label === null) {
				options.label = (this.type === "input" ? this.buttonElement
						.val() : this.buttonElement.html());
			}

			this._hoverable(this.buttonElement);

			this.buttonElement.addClass(baseClasses).attr("role", "button")
			...
			if (this.type === "checkbox") {
				...
				this.buttonElement.bind("mousedown" + this.eventNamespace,
						...
						// see #8559, we bind to blur here in case the button
						// element loses
						// focus between keydown and keyup, it would be left in
						// an "active" state
						.bind(
				...
				if (this.buttonElement.is("a")) {
					this.buttonElement.keyup(function(event) {
								if (event.keyCode === $.ui.keyCode.SPACE) {
									...
									// correctly (just as 2nd argument doesn't
									// work)
									$(this).click();
								}
							});
			...
			// $.Widget.prototype._setOptionDisabled so it's easy to proxy and
			// can
		...
		_destroy : function() {
			...
			this.buttonElement.removeClass(baseClasses + " " + stateClasses
		...
		refresh : function() {
			...
			if (this.type === "radio") {
				radioGroup(this.element[0]).each(function() {
					if ($(this).is(":checked")) {
						$(this).button("widget").addClass("ui-state-active")
								.attr("aria-pressed", "true");
					} else {
						$(this).button("widget").removeClass("ui-state-active")
				...
				if (this.element.is(":checked")) {
					...
					this.buttonElement.removeClass("ui-state-active").attr(
							"aria-pressed", "false");
				}
			}
		},
		...
		_resetButton : function() {
			...
			var buttonElement = this.buttonElement.removeClass(typeClasses), buttonText = $(
					...
					.appendTo(buttonElement.empty()).text(), icons = this.options.icons, multipleIcons = icons.primary
					&& icons.secondary, buttonClasses = [];

			if (icons.primary || icons.secondary) {
				if (this.options.text) {
					buttonClasses.push("ui-button-text-icon"
							+ (multipleIcons ? "s" : (icons.primary
									? "-primary"
									: "-secondary")));
				}
	...
	$.widget("ui.buttonset", {
		...
		refresh : function() {
			...
			this.buttons = this.element
					...
					.filter(":first").addClass(rtl
							? "ui-corner-right"
							: "ui-corner-left").end().filter(":last")
					.addClass(rtl ? "ui-corner-left" : "ui-corner-right").end()
					.end();
		},
...
(function($, undefined) {
	...
	$.extend(Datepicker.prototype, {
		 ...
		 * the target input field or division or span @param settings object -
		 * the new settings to use for this date picker instance (anonymous)
		 */
		...
		_connectDatepicker : function(target, inst) {
			...
			// If disabled option is true, disable the datepicker once it has
			// been attached to the input (see ticket #5665)
		...
		_attachments : function(input, inst) {
			var showOn, buttonText, buttonImage, appendText = this._get(inst,
					"appendText"), isRTL = this._get(inst, "isRTL");
			...
			if (showOn === "button" || showOn === "both") { // pop-up date
				...
				inst.trigger = $(this._get(inst, "buttonImageOnly")
						...
						: $("<button type='button'></button>")
								.addClass(this._triggerClass).html(!buttonImage
										? buttonText
										: $("<img/>").attr({
													src : buttonImage,
													alt : buttonText,
		...
		_autoSize : function(inst) {
			if (this._get(inst, "autoSize") && !inst.inline) {
				...
				if (dateFormat.match(/[DM]/)) {
					...
					date.setMonth(findMax(this._get(inst, (dateFormat
									.match(/MM/)
									? "monthNames"
									: "monthNamesShort"))));
		...
		_inlineDatepicker : function(target, inst) {
			...
			// If disabled option is true, disable the datepicker before showing
			// it (see ticket #5665)
			if (inst.settings.disabled) {
				this._disableDatepicker(target);
			}
		...
		/*
		 * Pop-up the date picker in a "dialog" box. @param input element -
		 * ignored @param date string or Date - the initial date to display
		 * @param onSelect function - the function to call when a date is
		 * selected @param settings object - update the dialog date picker
		 * instance's settings (anonymous object) @param pos int[2] -
		 * coordinates for the dialog's position within the screen or event -
		 * with x/y coordinates or leave empty for default (screen centre)
		...
		/*
		 * Detach a datepicker from its control. @param target element - the
		...
		/*
		 * Enable the date picker to a jQuery selection. @param target element -
		...
		/*
		 * Retrieve the instance data for the target control. @param target
		 * element - the target input field or division or span @return object -
		 * the associated instance data @throws error if a jQuery problem
		 * getting data
		 */
		 ...
		 * input field or division. @param target element - the target input
		 * field or division or span @param name object - the new settings to
		 * update or string - the name of the setting to change or retrieve,
		 * when retrieving also "all" for all instance settings or "defaults"
		 * for all global defaults @param value any - the new value for the
		 * setting (omit if above is an object or to retrieve a value)
		 */
		...
		/*
		 * Set the dates for a jQuery selection. @param target element - the
		 * target input field or division or span @param date Date - the new
		 * date
		 */
		...
		/*
		 * Get the date(s) for the first entry in a jQuery selection. @param
		 * target element - the target input field or division or span @param
		 * noDefault boolean - true if no default date is to be used @return
			...
			if (inst.input.val() !== inst.lastVal) {
				try {
					date = $.datepicker.parseDate($.datepicker._get(inst,
									"dateFormat"), (inst.input ? inst.input
									.val() : null), $.datepicker
		...
		/*
		 * Pop-up the date picker for a given input field. If false returned
		 * from beforeShow event handler do not show. @param input element - the
		 * input field attached to the date picker or event - if triggered by
		 * focus
		 */
		_showDatepicker : function(input) {
			...
			beforeShowSettings = beforeShow ? beforeShow.apply(input, [input,
							inst]) : {};
		...
		_updateDatepicker : function(inst) {
			this.maxRows = 4; // Reset the max number of rows being displayed
								// (see #7043)
		...
		_checkOffset : function(inst, offset, isFixed) {
			var dpWidth = inst.dpDiv.outerWidth(), dpHeight = inst.dpDiv
					...
					.outerWidth() : 0, inputHeight = inst.input ? inst.input
		...
		_findPos : function(obj) {
			...
			while (obj
					&& (obj.type === "hidden" || obj.nodeType !== 1 || $.expr.filters
							.hidden(obj))) {
		...
		/*
		 * Hide the date picker from view. @param input element - the input
		 * field attached to the date picker
		 */
		 ...
		 * @param date Date - the date to customise @return [boolean, string] -
		 * is this date selectable?, what is its CSS class?
		 */
		 ...
		 * ISO 8601 definition. @param date Date - the date to get the week for
		 ...
		 * string - the date in the above format @param settings Object -
		 * attributes include: shortYearCutoff number - the cutoff year for
		 * determining the century (optional) dayNamesShort string[7] -
		 * abbreviated names of the days from Sunday (optional) dayNames
		 * string[7] - names of the days from Sunday (optional) monthNamesShort
		 * string[12] - abbreviated names of the months (optional) monthNames
		 * string[12] - names of the months (optional) @return Date - the
		...
		parseDate : function(format, value, settings) {
			...
			var iFormat, dim, extra, iValue = 0, shortYearCutoffTemp = (settings
					? settings.shortYearCutoff
					: null)
					|| this._defaults.shortYearCutoff, shortYearCutoff = (typeof shortYearCutoffTemp !== "string"
					? shortYearCutoffTemp
					: new Date().getFullYear() % 100
							+ parseInt(shortYearCutoffTemp, 10)), dayNamesShort = (settings
					? settings.dayNamesShort
					: null)
					|| this._defaults.dayNamesShort, dayNames = (settings
					? settings.dayNames
					: null)
					|| this._defaults.dayNames, monthNamesShort = (settings
					? settings.monthNamesShort
					: null)
					|| this._defaults.monthNamesShort, monthNames = (settings
					? settings.monthNames
					: null)
			...
			getNumber = function(match) {
				var isDoubled = lookAhead(match), size = (match === "@"
						? 14
						: (match === "!" ? 20 : (match === "y" && isDoubled
								? 4
								: (match === "o" ? 3 : 2)))), digits = new RegExp("^\\d{1,"
						+ size + "}"), num = value.substring(iValue)
						.match(digits);
				if (!num) {
			...
			getName = function(match, shortNames, longNames) {
				...
				$.each(names, function(i, pair) {
					var name = pair[1];
					if (value.substr(iValue, name.length).toLowerCase() === name
							.toLowerCase()) {
						index = pair[0];
						iValue += name.length;
						return false;
		...
		_ticksTo1970 : (((1970 - 1) * 365 + Math.floor(1970 / 4)
				- Math.floor(1970 / 100) + Math.floor(1970 / 400))
				* 24 * 60 * 60 * 10000000),

		/*
		 * Format a date object into a string value. The format can be
		 * combinations of the following: d - day of month (no leading zero) dd -
		 * day of month (two digit) o - day of year (no leading zeros) oo - day
		 * of year (three digit) D - day name short DD - day name long m - month
		 * of year (no leading zero) mm - month of year (two digit) M - month
		 * name short MM - month name long y - year (two digit) yy - year (four
		 * digit) @ - Unix timestamp (ms since 01/01/1970) ! - Windows ticks
		 * (100ns since 01/01/0001) "..." - literal text '' - single quote
		 * 
		 * @param format string - the desired format of the date @param date
		 * Date - the date value to format @param settings Object - attributes
		 * include: dayNamesShort string[7] - abbreviated names of the days from
		 * Sunday (optional) dayNames string[7] - names of the days from Sunday
		 * (optional) monthNamesShort string[12] - abbreviated names of the
		 * months (optional) monthNames string[12] - names of the months
		 * (optional) @return string - the date in the above format
		 */
		formatDate : function(format, date, settings) {
			...
			var iFormat, dayNamesShort = (settings
					? settings.dayNamesShort
					: null)
					|| this._defaults.dayNamesShort, dayNames = (settings
					? settings.dayNames
		...
		_setDateFromField : function(inst, noDefault) {
			...
			var dateFormat = this._get(inst, "dateFormat"), dates = inst.lastVal = inst.input
					? inst.input.val()
		...
		_getDefaultDate : function(inst) {
			return this._restrictMinMax(inst, this._determineDate(inst, this
		...
		_determineDate : function(inst, date, defaultDate) {
			var offsetNumeric = function(offset) {
				...
				var date = (offset.toLowerCase().match(/^c/) ? $.datepicker
						._getDate(inst) : null)
						|| new Date(), year = date.getFullYear(), month = date
						.getMonth(), day = date.getDate(), pattern = /([+\-]?[0-9]+)\s*(d|D|w|W|m|M|y|Y)?/g, matches = pattern
						.exec(offset);
				...
				while (matches) {
					switch (matches[2] || "d") {
						...
						case "M" :
							...
							day = Math.min(day, $.datepicker._getDaysInMonth(
											year, month));
							break;
						case "y" :
						case "Y" :
				...
				return new Date(year, month, day);
					...
					: (typeof date === "string"
							...
							: (typeof date === "number" ? (isNaN(date)
									? defaultDate
									: offsetNumeric(date)) : new Date(date
									.getTime()))));
		...
		/*
		 * Handle switch to/from daylight saving. Hours may be non-zero on
		 * daylight saving cut-over: > 12 when midnight changeover, but then
		 * cannot generate midnight datetime, so jump to 1AM, otherwise reset.
		 * @param date (Date) the date to check @return (Date) the corrected
		 * date
		 */
		...
		_setDate : function(inst, date, noChange) {
			var clear = !date, origMonth = inst.selectedMonth, origYear = inst.selectedYear, newDate = this
					._restrictMinMax(inst, this._determineDate(inst, date,
									new Date()));
		...
		_generateHTML : function(inst) {
			var maxDraw, prevText, prev, nextText, next, currentText, gotoDate, controls, buttonPanel, firstDay, showWeek, dayNames, dayNamesMin, monthNames, monthNamesShort, beforeShowDay, showOtherMonths, selectOtherMonths, defaultDate, html, dow, row, group, col, selectedDate, cornerClass, calender, thead, day, daysInMonth, leadDays, curRows, numRows, printDate, dRow, tbody, daySettings, otherMonth, unselectable, tempDate = new Date(), today = this
					._daylightSavingAdjust(new Date(tempDate.getFullYear(),
									tempDate.getMonth(), tempDate.getDate())), // clear
																				// time
			isRTL = this._get(inst, "isRTL"), showButtonPanel = this._get(inst,
					"showButtonPanel"), hideIfNoPrevNext = this._get(inst,
					"hideIfNoPrevNext"), navigationAsDateFormat = this._get(
					inst, "navigationAsDateFormat"), numMonths = this
					._getNumberOfMonths(inst), showCurrentAtPos = this._get(
					inst, "showCurrentAtPos"), stepMonths = this._get(inst,
					"stepMonths"), isMultiMonth = (numMonths[0] !== 1 || numMonths[1] !== 1), currentDate = this
					._daylightSavingAdjust((!inst.currentDay ? new Date(9999,
							9, 9) : new Date(inst.currentYear,
							inst.currentMonth, inst.currentDay))), minDate = this
			...
			if (maxDate) {
				maxDraw = this._daylightSavingAdjust(new Date(maxDate
								.getFullYear(), maxDate.getMonth()
								- (numMonths[0] * numMonths[1]) + 1, maxDate
								.getDate()));
			...
			for (row = 0; row < numMonths[0]; row++) {
				...
				for (col = 0; col < numMonths[1]; col++) {
					selectedDate = this._daylightSavingAdjust(new Date(
					...
					calender += "<div class='ui-datepicker-header ui-widget-header ui-helper-clearfix"
							...
							+ this._generateMonthYearHeader(inst, drawMonth,
									drawYear, minDate, maxDate, row > 0
											|| col > 0, monthNames,
									monthNamesShort)
							+ // draw month headers
					...
					numRows = (isMultiMonth ? this.maxRows > curRows
							? this.maxRows
							: curRows : curRows); // If multiple months, use
						...
						for (dow = 0; dow < 7; dow++) { // create date picker
							...
							tbody += "<td class='"
									...
									((printDate.getTime() === selectedDate
											...
											// printedDate and defaultDate is
											// selectedDate
											" " + this._dayOverClass
											: "")
		...
		_generateMonthYearHeader : function(inst, drawMonth, drawYear, minDate,
			...
			var inMinYear, inMaxYear, month, years, thisYear, determineYear, year, endYear, changeMonth = this
					._get(inst, "changeMonth"), changeYear = this._get(inst,
		...
		_adjustInstDate : function(inst, offset, period) {
			var year = inst.drawYear + (period === "Y" ? offset : 0), month = inst.drawMonth
					+ (period === "M" ? offset : 0), day = Math.min(
					inst.selectedDay, this._getDaysInMonth(year, month))
					+ (period === "D" ? offset : 0), date = this
					._restrictMinMax(inst, this._daylightSavingAdjust(new Date(
									year, month, day)));
		...
		_getDaysInMonth : function(year, month) {
			return 32
					- this._daylightSavingAdjust(new Date(year, month, 32))
							.getDate();
		},
		...
		_canAdjustMonth : function(inst, offset, curYear, curMonth) {
			var numMonths = this._getNumberOfMonths(inst), date = this
					._daylightSavingAdjust(new Date(curYear, curMonth
									+ (offset < 0 ? offset : numMonths[0]
											* numMonths[1]), 1));
		...
		_isInRange : function(inst, date) {
			var yearSplit, currentYear, minDate = this._getMinMaxDate(inst,
					"min"), maxDate = this._getMinMaxDate(inst, "max"), minYear = null, maxYear = null, years = this
					._get(inst, "yearRange");
			if (years) {
	...
	function bindHover(dpDiv) {
		...
		return dpDiv.delegate(selector, "mouseout", function() {
			...
			if (!$.datepicker._isDisabledDatepicker(instActive.inline ? dpDiv
	...
	$.widget("ui.dialog", {
		...
		_destroy : function() {
			...
			this.element.removeUniqueId()
					...
					// Without detaching first, the following becomes really
					// slow
					.detach();
		...
		_createTitlebar : function() {
			...
			this._on(this.uiDialogTitlebar, {
						mousedown : function(event) {
							...
							// Focusing a dialog that is partially scrolled out
							// of view
...
(function($, undefined) {
	...
	$.effects.effect.blind = function(o, done) {
		...
		var el = $(this), props = ["position", "top", "bottom", "left",
				...
				.test(direction), ref = vertical ? "height" : "width", ref2 = vertical
				? "top"
...
(function($, undefined) {
	...
	$.effects.effect.clip = function(o, done) {
		...
		var el = $(this), props = ["position", "top", "bottom", "left",
				...
				|| "vertical", vert = direction === "vertical", size = vert
				? "height"
...
(function($, undefined) {
	...
	$.effects.effect.drop = function(o, done) {
		...
		var el = $(this), props = ["position", "top", "bottom", "left",
				...
				|| "left", ref = (direction === "up" || direction === "down")
				? "top"
				: "left", motion = (direction === "up" || direction === "left")
				? "pos"
				: "neg", animation = {
			opacity : show ? 1 : 0
		}, distance;
...
(function($, undefined) {
	...
	$.effects.effect.fold = function(o, done) {
		...
		var el = $(this), props = ["position", "top", "bottom", "left",
				"right", "height", "width"], mode = $.effects.setMode(el,
				o.mode || "hide"), show = mode === "show", hide = mode === "hide", size = o.size
				|| 15, percent = /([0-9]+)%/.exec(size), horizFirst = !!o.horizFirst, widthFirst = show !== horizFirst, ref = widthFirst
				? ["width", "height"]
...
(function($, undefined) {
	...
	$.effects.effect.scale = function(o, done) {
		...
		var el = $(this), options = $.extend(true, {}, o), mode = $.effects
				...
				10)
				|| (parseInt(o.percent, 10) === 0 ? 0 : (mode === "hide"
						? 0
	...
	$.effects.effect.size = function(o, done) {
		...
		mode = $.effects.setMode(el, o.mode || "effect"), restore = o.restore
				...
				|| ["middle", "center"], position = el.css("position"), props = restore
				? props0
				: props1, zero = {
			height : 0,
			width : 0,
					...
					if (position === "static") {
						...
						$.each(["top", "left"], function(idx, pos) {
							el.css(pos, function(_, str) {
										var val = parseInt(str, 10), toRef = idx
												? el.to.left
												: el.to.top;

										// if original was "auto", recalculate
										// the new value from wrapper
										if (str === "auto") {
											return toRef + "px";
										}
...
(function($, undefined) {
	...
	$.effects.effect.shake = function(o, done) {
		...
		var el = $(this), props = ["position", "top", "bottom", "left",
				"right", "height", "width"], mode = $.effects.setMode(el,
				o.mode || "effect"), direction = o.direction || "left", distance = o.distance
				|| 20, times = o.times || 3, anims = times * 2 + 1, speed = Math
				.round(o.duration / anims), ref = (direction === "up" || direction === "down")
				? "top"
...
(function($, undefined) {
	...
	$.effects.effect.transfer = function(o, done) {
		var elem = $(this), target = $(o.to), targetFixed = target
				.css("position") === "fixed", body = $("body"), fixTop = targetFixed
				? body.scrollTop()
	...
	$.widget("ui.menu", {
			...
			switch (event.keyCode) {
				...
				default :
					...
					if (!match.length) {
						character = String.fromCharCode(event.keyCode);
						regex = new RegExp("^" + escape(character), "i");
						match = this.activeMenu.children(".ui-menu-item")
								.filter(function() {
									return regex.test($(this).children("a")
											.text());
								});
					}
			...
			submenus
					...
					.hide().attr({
						...
						var menu = $(this), item = menu.prev("a"), submenuCarat = $("<span>")
								.addClass("ui-menu-icon ui-icon " + icon).data(
		...
		_scrollIntoView : function(item) {
			...
			if (this._hasScroll()) {
				borderTop = parseFloat($.css(this.activeMenu[0],
						"borderTopWidth"))
						|| 0;
				paddingTop = parseFloat($.css(this.activeMenu[0], "paddingTop"))
						|| 0;
		...
		collapseAll : function(event, all) {
			...
			this.timer = this._delay(function() {
						...
						// contains the event
						var currentMenu = all ? this.element : $(event
								&& event.target).closest(this.element
								.find(".ui-menu"));
		...
		_close : function(startMenu) {
			...
			startMenu.find(".ui-menu").hide().attr("aria-hidden", "true").attr(
		...
		_move : function(direction, filter, event) {
			...
			if (this.active) {
				if (direction === "first" || direction === "last") {
					next = this.active[direction === "first"
							? "prevAll"
							: "nextAll"](".ui-menu-item").eq(-1);
				} else {
		...
		nextPage : function(event) {
			...
			if (this._hasScroll()) {
				...
				this.focus(event,
						this.activeMenu.children(".ui-menu-item")[!this.active
								? "first"
								: "last"]());
			}
		},
	...
	$.fn.position = function(options) {
		...
		$.each(["my", "at"], function() {
			...
			if (pos.length === 1) {
				pos = rhorizontal.test(pos[0])
						? pos.concat(["center"])
						: rvertical.test(pos[0]) ? ["center"].concat(pos) : [
								"center", "center"];
			}
			...
			offsets[this] = [horizontalOffset ? horizontalOffset[0] : 0,
		...
		return this.each(function() {
			var collisionPosition, using, elem = $(this), elemWidth = elem
					...
					this, "marginLeft"), marginTop = parseCss(this, "marginTop"), collisionWidth = elemWidth
					+ marginLeft
					+ parseCss(this, "marginRight")
					+ scrollInfo.width, collisionHeight = elemHeight
					+ marginTop + parseCss(this, "marginBottom")
					+ scrollInfo.height, position = $.extend({}, basePosition), myOffset = getOffsets(
	...
	$.ui.position = {
		fit : {
			left : function(position, data) {
				var within = data.within, withinOffset = within.isWindow
						? within.scrollLeft
		...
		flip : {
			left : function(position, data) {
				var within = data.within, withinOffset = within.offset.left
						+ within.scrollLeft, outerWidth = within.width, offsetLeft = within.isWindow
						? within.scrollLeft
						...
						+ data.collisionWidth - outerWidth - offsetLeft, myOffset = data.my[0] === "left"
						? -data.elemWidth
			...
			top : function(position, data) {
				var within = data.within, withinOffset = within.offset.top
						+ within.scrollTop, outerHeight = within.height, offsetTop = within.isWindow
						? within.scrollTop
						...
						+ data.collisionHeight - outerHeight - offsetTop, top = data.my[1] === "top", myOffset = top
						? -data.elemHeight
...
(function($, undefined) {
	...
	$.widget("ui.progressbar", {
		...
		_constrainedValue : function(newValue) {
			...
			return this.indeterminate ? false : Math.min(this.options.max, Math
							.max(this.min, newValue));
		},
		...
		_refreshValue : function() {
			...
			this.valueDiv.toggle(this.indeterminate || value > this.min)
			...
			if (this.indeterminate) {
				...
				if (!this.overlayDiv) {
					this.overlayDiv = $("<div class='ui-progressbar-overlay'></div>")
	...
	$.widget("ui.slider", $.ui.mouse, {
		...
		_handleEvents : {
			keydown : function(event) {
				...
				var allowed, curVal, newVal, step, index = $(event.target)
	...
	$.widget("ui.spinner", {
		...
		_draw : function() {
			var uiSpinner = this.uiSpinner = this.element
					.addClass("ui-spinner-input").attr("autocomplete", "off")
			...
			// button bindings
			this.buttons = uiSpinner.find(".ui-spinner-button").attr(
					"tabIndex", -1).button().removeClass("ui-corner-all");
		...
		_increment : function(i) {
			...
			if (incremental) {
				return $.isFunction(incremental) ? incremental(i) : Math
						.floor(i * i * i / 50000 - i * i / 500 + 17 * i / 200
								+ 1);
			}
		...
		_setOption : function(key, value) {
			...
			if (key === "icons") {
				this.buttons.first().find(".ui-icon")
						.removeClass(this.options.icons.up).addClass(value.up);
		...
		_parse : function(val) {
			if (typeof val === "string" && val !== "") {
				val = window.Globalize && this.options.numberFormat ? Globalize
						.parseFloat(val, 10, this.options.culture) : +val;
			}
		...
		_format : function(value) {
			...
			return window.Globalize && this.options.numberFormat ? Globalize
		...
		_destroy : function() {
			this.element.removeClass("ui-spinner-input")
					.prop("disabled", false).removeAttr("autocomplete")
					.removeAttr("role").removeAttr("aria-valuemin")
					.removeAttr("aria-valuemax").removeAttr("aria-valuenow");
	...
	$.widget("ui.tabs", {
		...
		_create : function() {
			...
			this.element
					...
					.delegate(".ui-tabs-nav > li",
							"mousedown" + this.eventNamespace, function(event) {
					...
					// focused
					// element since clicking on a non-focusable element should
					// focus
					// the body anyway.
			...
			if ($.isArray(options.disabled)) {
				options.disabled = $.unique(options.disabled.concat($.map(
		...
		_tabKeydown : function(event) {
			...
			if (!event.ctrlKey) {
				...
				// already selected.
				// Otherwise AT may confuse the user by stating that they need
				// to activate the tab,
		...
		_handlePageNav : function(event) {
			...
			if (event.altKey && event.keyCode === $.ui.keyCode.PAGE_DOWN) {
				this._activate(this
						._focusNextTab(this.options.active + 1, true));
				return true;
			}
		},
		...
		refresh : function() {
			...
			if (options.active === false || !this.anchors.length) {
				...
				if (this.tabs.length === options.disabled.length) {
					...
					this._activate(this._findNextTab(Math.max(0, options.active
											- 1), false));
				}
		...
		_processTabs : function() {
			...
			this.anchors.each(function(i, anchor) {
				var selector, panel, panelId, anchorId = $(anchor).uniqueId()
						.attr("id"), tab = $(anchor).closest("li"), originalAriaControls = tab
						.attr("aria-controls");
		...
		_setupHeightStyle : function(heightStyle) {
			...
			if (heightStyle === "fill") {
				...
				this.panels.each(function() {
					maxHeight = Math
							.max(maxHeight, $(this).height("").height());
				}).height(maxHeight);
			}
		},
		...
		_eventHandler : function(event) {
			var options = this.options, active = this.active, anchor = $(event.currentTarget), tab = anchor
					...
					&& options.collapsible, toShow = collapsing ? $() : this
					._getPanelForTab(tab), toHide = !active.length ? $() : this
		...
		_toggle : function(event, eventData) {
			...
			// If we're opening from collapsed state, remove the previous tab
			// from the tab order.
		...
		_destroy : function() {
			...
			this.anchors.removeClass("ui-tabs-anchor").removeAttr("role")
					.removeAttr("tabIndex").removeUniqueId();
			...
			this.tabs.add(this.panels).each(function() {
				if ($.data(this, "ui-tabs-destroy")) {
					...
					$(this)
							...
							.removeAttr("tabIndex").removeAttr("aria-live")
		...
		load : function(index, event) {
			...
			var that = this, tab = this.tabs.eq(index), anchor = tab
					.find(".ui-tabs-anchor"), panel = this._getPanelForTab(tab), eventData = {
				tab : tab,
				panel : panel
			};
	...
	$.widget("ui.tooltip", {
		...
		_disable : function() {
			...
			this.element.find(this.options.items).addBack().each(function() {
				var element = $(this);
				if (element.is("[title]")) {
					element.data("ui-tooltip-title", element.attr("title"))
							.attr("title", "");
				}
			});
		},
		...
		_updateContent : function(target, event) {
			var content, contentOption = this.options.content, that = this, eventType = event
					? event.type
					: null;
		...
		_open : function(event, target, content) {
			var tooltip, events, delayedShow, positionOption = $.extend({},
					this.options.position);
		...
		close : function(event) {
			var that = this, target = $(event
					? event.currentTarget
					: this.element), tooltip = this._find(target);
